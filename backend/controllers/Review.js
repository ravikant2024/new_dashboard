...
exports.getAllReviews = async (req, res) => {
  try {
    ...
    const totalDocs = await Review.countDocuments().exec();
    let query = Review.find({}).populate('user', '-password').populate('product');
console.log("query",query)
    ...
    console.log(error);
    res.status(500).json({ message: 'Error getting all reviews, please try again later' });
  }
};
